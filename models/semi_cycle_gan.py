...
#             self.visuals_names.extend(['real_shift_A', 'fake_shift_A', 'real_shift_B', 'fake_shift_B'])
        ...
        if self.opt.use_pretrain_img2depth:
            ...
            if self.opt.l_hole_A > 0 and self.isTrain:
...
#                 weight_class = torch.tensor([3.0]).to(self.device)   #HYPERPARAM
            ...
            self.hole_border = self.opt.hole_border 
            self.calc_l_step()
...
#             self.gaus_blur = GaussianSmoothing(1, 7, 10, self.device)  #HYPERPARAM
    ...
    def forward(self):
        ...
        ###Fake depth
        if self.opt.use_semantic and self.isTrain:
            self.fake_depth_B, self.fake_semantic_A = self.netG_A(self.real_depth_A, self.img_feature_A, return_logits=True)
        else:
            self.fake_depth_B = self.netG_A(self.real_depth_A, self.img_feature_A)
        ...
        if self.opt.use_semi_cycle and self.isTrain:
            ...
            if self.opt.use_cycle_B:
                ...
                self.rec_depth_B = self.netG_A(self.fake_depth_A, self.img_feature_B)
        ...
        if self.isTrain:
            if self.opt.l_identity > 0:
                self.idt_A = self.netG_A(self.real_depth_B, self.img_feature_B)
...
#         fake_B = self.fake_B_pool
    ...
    def backward_G(self):
        ...
        if self.opt.disc_for_normals:
            self.loss_G_A = self.loss_G_A + self.criterionGAN(self.netD_A_normal(self.fake_norm_B), True)
        ...
        if self.opt.use_cycle_A:
            ...
            self.loss_cycle_n_A = self.criterionMaskedCosSim(self.rec_norm_A, self.real_norm_A, ~self.hole_mask_A.repeat(1,3,1,1)) * 10 * self.l_cycle_A
        ...
        if self.opt.use_cycle_B:
            ...
            self.loss_cycle_n_B = self.criterionCosSim(self.rec_norm_B, self.real_norm_B) * 10 * self.l_cycle_B
...
#         self.test_cycle_A = self.criterionMaskedL1(util.data_to_meters(self.real_depth_A, self.opt), util.data_to_meters(self.rec_depth_A, self.opt) , ~self.hole_mask_A)
#         self.test_cycle_B = self.criterionMaskedL1(util.data_to_meters(self.real_depth_B, self.opt), util.data_to_meters(self.rec_depth_B, self.opt) , ~self.hole_mask_B)
