...
def get_activations(depths_list, normals_list, model, batch_size=50, dims=2048,
    ...
    if batch_size > len(depths_list):
        print(('Warning: batch size is bigger than the data size. '
               'Setting batch size to data size'))
        batch_size = len(depths_list)

    pred_arr = np.empty((len(depths_list), dims))

    for i in tqdm(range(0, len(depths_list), batch_size)):
        if verbose:
...
def calculate_given_paths(paths, batch_size, cuda, dims):
    ...
    model = InceptionV3([block_idx], use_fid_inception=True, resize_input=False, normalize_input=True)
