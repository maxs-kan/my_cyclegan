...
class SRDataset(BaseDataset):
    ...
    def load_data(self, index):
        ...
        if self.opt.datasets == 'Scannet_Scannet':
            if self.opt.phase != 'test':
                B_h_start, B_h_stop, B_w_start, B_w_stop = self.crop_indx(B_depth_n, sr=True)
                if self.opt.phase == 'train':
    ...
    def transform(self, domain, depth, img):
        ...
        if self.opt.phase == 'train':
            if domain == 'A':
                img, depth, crop = self.random_crop(img, depth, self.opt.crop_size_h_A, self.opt.crop_size_w_A)
            elif domain == 'B':
                img, depth, crop = self.random_crop(img, depth, self.opt.crop_size_h_B, self.opt.crop_size_w_B)
        else:
            crop = [0]
